...
- **`languages` is PROVIDER-level; a model row may override the count.** When one provider's models do not share a language table, `languages.codes` is their UNION and each `models[]` row carries its own optional `languageCount` (integer, upstream's own count for that model). Anything that makes a PER-MODEL claim — the /choosing-a-model page, a per-model caption — must read `languageCount` and fall back to `languages.count` only when the row omits it. `azureMaiTranscribe` is the live case: 60 for `mai-transcribe-2`, 42 for `mai-transcribe-1.5`. Per-model CODE LISTS are deliberately NOT duplicated here — they live in `shared-models/models-catalog.json` (`supportedLanguages`), folded to the picker code space, and that is what every picker and Model Library narrows on.
