...
- Vendor-claimed counts can lie. xAI docs say "24" but list 25; Whisper docs say "99" but the tokenizer has 100. The catalog reports the array length, not the marketing number. Azure MAI-Transcribe 1.5 used to be an example of this — prose said "43 languages" over a 42-row table — and is no longer: the table gained `zh` and now lists 43, so the two agree. Re-read the table before trusting a count this file already carries.
</important>
