...
extension RecordingTranscriptionFlow {
    ...
    func handleStopRecordingWithTranscription(mode: String, cancelled: Bool) async {
        ...
        // PRIVACY (issue #795): the breadcrumbs in this function report the Mode's
        // UUID, never its name. The name is free text the user typed; the id is
        // generated by the app (`AppState.selectedModeId` is always
        // `Mode.id?.uuidString`), so it is not user content, and it is the only
        // mode identity in scope before the Mode row is resolved further down.
        let reportedModeId = sessionModeId.isEmpty ? "none" : sessionModeId
        ...
        SentryService.addBreadcrumb(
            ...
            data: [
                ...
                // `modeId` is the non-content replacement (see `reportedModeId`).
            ...
            if AppLogger.isErrorLoggingEnabled {
                ...
                SentryService.addBreadcrumb(
                    ...
                    data: [
                        ...
                        // The Mode's name is user-typed free text and is never
                        // reported (issue #795); `modeId` replaces it.
                        "modeId": reportedModeId,
        ...
        if !isReadable {
            ...
            if AppLogger.isErrorLoggingEnabled {
                // PRIVACY (issue #795): no PATH is reported here. The Mode's name is
                // user-typed free text, and a path under the user's home is the same
                // class of value — the default recordings folder is
                // `~/Documents/hyperwhisper/recordings`, so both the audio path and
                // the folder carry the account name. `transcriptionFailureExtras`
                // shipped real user names for months for exactly this reason.
                // The booleans below are the established replacement in this flow
                // (`RecordingTranscriptionFlow+ErrorHandling.swift`,
                // `recordingStartFailureMetadata`): they carry what the paths were
                ...
                SentryService.addBreadcrumb(
                    ...
                    data: [
                        ...
                        "audioFileSizeBytes": unreadableFileSize,
                        "audioFileExtension": audioURL.pathExtension,
                        "recordingsFolderIsDefault": !recordingsFolder.isEmpty && recordingsFolder == defaultRecordingsFolder,
                        "recordingsFolderIsInHome": recordingsFolder.hasPrefix(NSHomeDirectory() + "/"),
                        "recordingsFolderWritable": !recordingsFolder.isEmpty && FileManager.default.isWritableFile(atPath: recordingsFolder)
                    ]
                )
            }
            await MainActor.run {
        ...
        do {
            ...
            if AppLogger.isErrorLoggingEnabled {
                ...
                SentryService.addBreadcrumb(
                    ...
                    data: [
                        // PRIVACY (issue #795): no PATH and no Mode NAME. A path under
                        // the user's home carries the account name, and the three
                        // file facts below are what the path was actually read for.
                        // `modeId` and `modePreset` are the non-content mode identity;
                        // `modePreset` is already normalised onto the closed
                        // `PresetType` set by the `modePreset` local above, so this
                        // breadcrumb still says WHICH KIND of Mode ran.
                        "modeId": reportedModeId,
