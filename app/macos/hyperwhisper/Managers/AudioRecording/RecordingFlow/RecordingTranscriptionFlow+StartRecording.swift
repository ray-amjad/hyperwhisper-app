    ...
    func handleStartRecording(mode: String) async {
        ...
        SentryService.addBreadcrumb(
            ...
            data: [
                ...
                // (issue #795). No non-content mode identity is in scope YET at this
                // point — `selectedModeId` is resolved further down this function and
                // the Mode row later still — and a Core Data fetch here would sit on
                // the recording hot path. So this breadcrumb carries `attemptId` only;
                // the "Started recording" log line below reports `modeId` for the same
                // attempt, and the stop-flow breadcrumbs carry it throughout.
        ...
        // PRIVACY (issue #795): the Mode's NAME is not logged, at any level. This
        // line is `.info` and `AppLogger.getRecentLogs` runs `log show` without
        // `--info`, so it does not reach the `recent_logs` extra TODAY — but that
        // is a flag on one command, not a guarantee, and the same argument this
        // branch refuses for `beforeSend` applies here. The Mode's UUID is
        // generated by the app, not typed by the user, so it is reported instead
        // and at `.public`: it is what makes a support log line identify the Mode
        // without publishing what the user called it. `selectedModeId` is already
        // in scope (resolved above), so this costs no Core Data fetch on the
        // recording hot path.
        let reportedModeId = selectedModeId.isEmpty ? "none" : selectedModeId
        AppLogger.audio.info("⏺️ Started recording · modeId=\(reportedModeId, privacy: .public)")
