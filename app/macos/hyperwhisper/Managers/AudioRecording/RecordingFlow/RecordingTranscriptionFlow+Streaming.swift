...
extension RecordingTranscriptionFlow {
    ...
    func stopStreamingTranscription(mode: String) async {
        ...
        let reportedModeId = activeSessionModeId.isEmpty ? "none" : activeSessionModeId
        SentryService.addBreadcrumb(
            message: "Streaming stop requested",
            category: "audio.streaming",
            data: [
                // PRIVACY (issue #795): the Mode's NAME is user-typed free text and
                // is never reported. The Mode's UUID is generated by the app, so it
                // is reported in its place.
                //
                // Do NOT rewrite this as "the transcription-failure event carries
                // `modePreset`". It does not, on this path: the streaming stop flow
                // never calls `TranscriptionPipeline.transcribeWithDetails` (its five
                // callers are `+StopRecording`, `+ErrorHandling`,
                // `FileTranscriptionFlow`, `TranscriptActionHandler` and
                // `TranscriptionRetryController`), so a streaming session produces no
                // transcription-failure event, no `modePreset` extra and no
                // `slow_transcription_completed` breadcrumb. `modeId` is therefore the
                // ONLY mode identity streaming diagnostics have; reading the preset
                // here would mean a new Core Data fetch this path does not otherwise
                // make.
                "modeId": reportedModeId,
                "attemptId": currentRecordingAttemptId ?? "none"
            ]
        )
