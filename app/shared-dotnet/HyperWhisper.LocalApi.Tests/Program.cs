...
static async Task DefaultModeInvariant()
{
    ...
    // The full required body, not `{"name":…}`: `POST /modes` requires seven
    // keys (issue #356 Decision B), and a body missing them answers HTTP 200
    // carrying a failure envelope — which a status check alone reads as success.
    using var seed = new StringContent(ModeBody("Hyper"), Encoding.UTF8, "application/json");
    var seedResponse = await fixture.Client.PostAsync("/modes", seed);
    Assert(seedResponse.StatusCode == HttpStatusCode.OK && !await HasFailureEnvelope(seedResponse),
        "seed mode create failed");
    ...
    using var second = new StringContent(ModeBody("Mine"), Encoding.UTF8, "application/json");
    var secondResponse = await fixture.Client.PostAsync("/modes", second);
    Assert(secondResponse.StatusCode == HttpStatusCode.OK && !await HasFailureEnvelope(secondResponse),
        "second mode create failed");
