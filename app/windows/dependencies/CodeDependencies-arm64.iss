[Code]
// ARM64 version of InnoDependencyInstaller
// Based on https://github.com/DomGries/InnoDependencyInstaller
// Modified for ARM64 Windows support
...
function Dependency_IsNetCoreInstalledArm64(Runtime: String; Major, Minor, Revision: Word): Boolean;
var
  Path: String;
  ResultCode: Integer;
  Output: TExecOutput;
...
begin
  // Check ARM64 registry path
  if not RegQueryStringValue(HKLM, 'SOFTWARE\dotnet\Setup\InstalledVersions\arm64', 'InstallLocation', Path) or not FileExists(Path + 'dotnet.exe') then begin
    Path := ExpandConstant('{commonpf64}') + '\dotnet\';
  end;
...
procedure Dependency_AddDotNet100DesktopArm64;
begin
  // https://dotnet.microsoft.com/download/dotnet/10.0
  if not Dependency_IsNetCoreInstalledArm64('Microsoft.WindowsDesktop.App', 10, 0, 0) then begin
    Dependency_Add('dotnet100desktop_arm64.exe',
      '/lcid ' + IntToStr(GetUILanguage) + ' /passive /norestart',
      '.NET Desktop Runtime 10.0 (ARM64)',
      'https://builds.dotnet.microsoft.com/dotnet/WindowsDesktop/10.0.9/windowsdesktop-runtime-10.0.9-win-arm64.exe',
      '', False, False);
  end;
end;

procedure Dependency_AddDotNet100AspNetCoreArm64;
begin
  // https://dotnet.microsoft.com/download/dotnet/10.0
  // Required by the in-app Local API server (Kestrel). See the matching
  // x64 procedure in CodeDependencies.iss for the why.
  if not Dependency_IsNetCoreInstalledArm64('Microsoft.AspNetCore.App', 10, 0, 0) then begin
    Dependency_Add('dotnet100aspnetcore_arm64.exe',
      '/lcid ' + IntToStr(GetUILanguage) + ' /passive /norestart',
      'ASP.NET Core Runtime 10.0 (ARM64)',
      'https://builds.dotnet.microsoft.com/dotnet/aspnetcore/Runtime/10.0.9/aspnetcore-runtime-10.0.9-win-arm64.exe',
      '', False, False);
  end;
end;
