...
procedure Dependency_AddDotNet100AspNetCore;
begin
  // https://dotnet.microsoft.com/download/dotnet/10.0
  // Required by the in-app Local API server (Kestrel via
  // <FrameworkReference Include="Microsoft.AspNetCore.App" />). The Desktop
  // runtime does NOT include ASP.NET Core, so without this guard the app
  // fails to launch on clean machines.
  if not Dependency_IsNetCoreInstalled('Microsoft.AspNetCore.App', 10, 0, 0) then begin
    Dependency_Add('dotnet100aspnetcore' + Dependency_ArchSuffix + '.exe',
      '/lcid ' + IntToStr(GetUILanguage) + ' /passive /norestart',
      'ASP.NET Core Runtime 10.0' + Dependency_ArchTitle,
      Dependency_String('https://builds.dotnet.microsoft.com/dotnet/aspnetcore/Runtime/10.0.9/aspnetcore-runtime-10.0.9-win-x86.exe', 'https://builds.dotnet.microsoft.com/dotnet/aspnetcore/Runtime/10.0.9/aspnetcore-runtime-10.0.9-win-x64.exe'),
      '', False, False);
  end;
end;
...
begin
  ...
  // Required by the /MD-linked parakeet-engine.exe (and sherpa-onnx-c-api.dll),
  // which import VCRUNTIME140.dll, VCRUNTIME140_1.dll, and MSVCP140.dll. On a
  // fresh Windows box without any C++ app installed, the loader cannot find
  // these and aborts the daemon at load time, surfacing as a misleading READY
  // timeout instead of an actionable error. The checked-in x64 sherpa DLL was
  // built with MSVC 14.44, so older 14.x redists are not sufficient.
  if RegQueryStringValue(HKLM64, 'SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\x64', 'Version', Version) and (Copy(Version, 1, 1) = 'v') then begin
    Delete(Version, 1, 1);
  end;
  if not StrToVersion(Version, PackedVersion) or (ComparePackedVersion(PackedVersion, PackVersionComponents(14, 44, 0, 0)) < 0) then begin
