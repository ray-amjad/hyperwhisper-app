...
- **Google Gemini 3.5 Transcribe** — HyperWhisper sends them in the `custom_vocabulary` field of the request. This is a real field of the API, and not text in a prompt. The limit is 100 terms, and the app cuts each term to 80 characters. The same limits apply through HyperWhisper Cloud and with your own Google key.
- **Grok STT** (SpaceXAI) — HyperWhisper sends them as `keyterm` hints, to a maximum of 100 terms. It removes terms of more than 50 characters.
- **Mistral Voxtral** — HyperWhisper sends them as a structured `context_bias` list, to a maximum of 100 terms. This applies both through HyperWhisper Cloud and with your own Mistral key.
