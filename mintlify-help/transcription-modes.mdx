...
<Note>
  Meta's Muse model supports real-time streaming upstream. HyperWhisper currently supports only its pre-recorded push-to-talk path. Muse therefore appears in a mode's Model row, but it does not appear in the HyperWhisper Live engine picker. A Muse mode costs 3 credits per audio minute.
</Note>
