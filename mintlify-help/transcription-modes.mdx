...
<Note>
  One mode is your **default mode**, and its name is fixed: when you edit it, the **Name** field is not editable, and the editor says why. Every other mode can be renamed. On a new install, the default mode is called **Default** on macOS and **Hyper** on Windows and Linux — but it is the mode's default status that fixes the name, not what it is called, so a default restored from a backup under another name is still the one you cannot rename.
