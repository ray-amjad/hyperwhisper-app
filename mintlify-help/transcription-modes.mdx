...
<Note>
  One mode is your **default mode**, and its name is fixed. When you edit it,
  the **Name** field is not editable, and the editor says why. Every other mode
  can be renamed. On a new install, the default mode is called **Hyper** on
  macOS, Windows, and Linux. The mode's default status fixes the name. A default
  restored from a backup under another name is still the one you cannot rename.

  Exactly one mode is the default, always. Delete the default mode and the next mode in the list takes its place. Restore a backup that carries two default modes, or none, and the app repairs it when it starts.
</Note>
...
|---|---|
| AssemblyAI | Universal-3.5 Pro, Universal-2, Dictation |
...
<Note>
  The SpaceXAI row gives two models. **Grok Voice Transcribe 2** is the model that the row selects first. **Grok Voice Transcribe 1** is the older model, at the same rate. A mode that you saved before this change stored no model id. HyperWhisper sends Grok Voice Transcribe 2 for that mode. Select **Grok Voice Transcribe 1** in the Model row if you want the older model.
</Note>

<Note>
  **AssemblyAI → Dictation** is a different model from Universal-3.5 Pro and Universal-2. It needs an explicit language, it does not accept Medical Mode, and it takes recordings up to 120 seconds. Read [AssemblyAI Dictation](/choosing-a-provider#assemblyai-dictation).
</Note>
