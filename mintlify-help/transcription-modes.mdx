...
After you select an engine, most engines show a second model picker. For example: Deepgram (Nova-3 and legacy Nova-2, each with General and Medical variants), ElevenLabs (Scribe v2), OpenAI (GPT-4o Transcribe, GPT-4o Mini Transcribe, GPT Transcribe, Whisper), Groq (Whisper Large v3 Turbo and v3), and AssemblyAI (Universal-3.5 Pro, legacy Universal-3 Pro, and Universal-2).

### Your Provider (BYOK)

Select a cloud provider. Then enter your own API key in **Model Library → API Keys**. The model list changes with the provider that you select.
