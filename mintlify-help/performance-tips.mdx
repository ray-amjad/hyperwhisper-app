...
<Tabs>
  ...
  <Tab title="Windows">
    Local Gemma post-processing is available on Windows x64. GPU acceleration uses NVIDIA CUDA and discrete AMD/Intel GPUs via Vulkan; integrated GPUs and ARM64 systems run on CPU.
    ...
    <Note>
      On integrated GPUs (including AMD APUs and Intel UHD/Iris), local Gemma post-processing uses CPU fallback. Transcription itself can still use GPU acceleration independently.
    </Note>
  </Tab>
</Tabs>
