...
| Limit | Value |
| --- | --- |
| Request body | 50 MiB (52,428,800 bytes) |
| Audio | 48 MiB (50,331,648 bytes) |
...
- **macOS** applies both limits. The request limit applies to every route that
  reads a body. The audio limit applies to the decoded bytes of `audio_base64`.
  It does not apply to a `file` path, because the server reads that file from
  disk in small pieces. macOS always answers with the HTTP 200 envelope.
- **Linux** applies both limits. It also applies the audio limit to a multipart
  `audio` part and to a `file` path. Only `/transcribe` answers with the
  HTTP 200 envelope. On the other routes, the web server stops the body itself
  and returns a bare **413** with no envelope.
