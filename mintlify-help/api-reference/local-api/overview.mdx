...
The audio limit applies to the decoded bytes of `audio_base64`, and to a
multipart `audio` part. These are the two shapes that the server holds in
memory. A `file` path is different: the server reads it from disk in small
pieces, and the audio limit does not apply to it.
...
The server does not return **413**. `PAYLOAD_TOO_LARGE` is not one of the
codes in the closed set, and a client that shares the macOS decoder cannot read
an envelope that contains it. Read the `code`, not the status.

These two limits apply on macOS. The Windows server does not apply them yet.
