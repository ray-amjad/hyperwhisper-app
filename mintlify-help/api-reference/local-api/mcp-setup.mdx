...
| `search_recordings` | Substring search across past transcripts. Accepts optional `since`, `until`, and `limit` filters. |
...
**`INVALID_REQUEST`** — a generic bad-request code, most commonly seen as an HTTP 401 when the bearer token in the discovery file no longer matches what the server expects (usually after **Regenerate token** in Settings — the bridge re-reads the file on every request, so the next agent call should succeed). It's also returned for malformed calls, such as a missing required field or passing `post_process_preset`/`post_process_prompt` to `transcribe` without `output_path`.
