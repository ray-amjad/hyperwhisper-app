...
<Note>
  This page is for macOS. For Windows, read [Windows Permissions](/windows-permissions). For Linux, read [Install HyperWhisper on Linux](/linux-installation), which covers the `hyperwhisper-input` group for global shortcuts and automatic paste, and the optional GNOME and KDE companions for active-application matching.
</Note>
...
<Steps>
  ...
  <Step title="Go back to HyperWhisper">
    The new permission takes effect while the app runs. You do not need to start the app again. Auto-paste examines the permission at each use, so it works at your next transcription. Bare-modifier push-to-talk starts as soon as the app finds the grant.
  </Step>
</Steps>

<Note>
  HyperWhisper watches for the Accessibility grant for 10 minutes after it finds the permission missing. If you take more than 10 minutes to grant it, select the push-to-talk mode again in **Settings → Shortcuts**, or start the app again.
</Note>
