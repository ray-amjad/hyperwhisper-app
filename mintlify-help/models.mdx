...
| **NVIDIA Nemotron 3.5** | macOS · Windows | 6 Latin · ~40 incl. Chinese, Japanese, Korean, Arabic (macOS); smaller English+Japanese streaming variant on Windows | Best on-device accuracy and the **broadest offline language coverage** on macOS — the only local option that reaches beyond European languages. |
...
| Model | Size (Windows) | Recommended VRAM | Languages | Best for |
| --- | --- | --- | --- | --- |
...
| Large v3 Turbo | 1.5 GB (Windows) / ~809 MB (macOS) | ~6 GB | Multilingual | Near-Large accuracy, much faster |
...
<Note>
  Except where noted above, the sizes in this table are Windows values. macOS builds of the same models are meaningfully smaller (for example, Large v3 Turbo is ~809 MB on macOS vs. 1.5 GB on Windows) — check the exact download size for your platform in **Model Library** before downloading.
</Note>
...
| Nemotron 3.5 (Multilingual) | macOS | ~1.3 GB | ~40 languages incl. Chinese, Japanese, Korean, Arabic | Broadest offline language coverage |
...
HyperWhisper Cloud is built-in — no API key, no separate account. It routes to best-in-class providers behind four featured accuracy tiers with automatic failover, and **you only pay for actual speech** (silence and empty recordings cost 0 credits). Use it when you want the highest accuracy without any setup.
...
| **Fast** | Groq Whisper Large v3 Turbo | Sub-second latency for English & major European languages |

These four are the featured picks with automatic failover to a backup provider. HyperWhisper Cloud actually offers all 11 supported providers (also including Soniox, Azure MAI-Transcribe, Google Chirp 3, OpenAI, AssemblyAI, Mistral Voxtral, and Gemini) selectable with no key required — providers outside the featured four don't get automatic failover. See [Providers](/choosing-a-provider) for pricing, cost examples, and per-language guidance.
