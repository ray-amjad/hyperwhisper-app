...
<Note>
  A Meta Muse mode still connects only to `transcribe-prod-v2.hyperwhisper.com`. The HyperWhisper Cloud server connects to `https://api.meta.ai/v1/asr/transcribe`. Do not add `api.meta.ai` to a device allowlist for HyperWhisper Cloud. HyperWhisper does not support a direct Meta API key or a Meta live WebSocket.
</Note>
...
Meta Muse is not in this section. It is available through HyperWhisper Cloud only.
