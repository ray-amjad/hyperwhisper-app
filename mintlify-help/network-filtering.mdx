...
| Domain | Purpose |
|--------|---------|
| `www.hyperwhisper.com` | Main website, API backend, update feed (appcast), and license validation |
| `builds.hyperwhisper.com` | Download host for the update installers (`.dmg` on macOS, `.exe` on Windows) |
...
- `wss://transcribe-prod-v2.hyperwhisper.com/ws/streaming-deepgram` - [Streaming transcription](/streaming-settings) with HyperWhisper Cloud as the streaming provider, and the live engine set to Deepgram Nova 3 (the default)
...
<Note>
  A HyperWhisper Cloud Meta Muse mode connects to `transcribe-prod-v2.hyperwhisper.com`. A direct Meta mode connects from your device to `https://api.meta.ai/v1/asr/transcribe`. HyperWhisper does not support a Meta live WebSocket.
</Note>
...
If you configured your own Meta Model API key, whitelist `api.meta.ai`. Direct Meta requests do not reach HyperWhisper servers.
...
- `wss://api.openai.com/v1/realtime` - [Streaming transcription](/streaming-settings) with OpenAI as the streaming provider
...
| `dictation.assemblyai.com` | Dictation with built-in cleanup, up to 120 seconds |
...
- `wss://api.elevenlabs.io/v1/speech-to-text/realtime` - [Streaming transcription](/streaming-settings) with ElevenLabs as the streaming provider
...
| Domain | Purpose |
|--------|---------|
| `generativelanguage.googleapis.com` | The dedicated speech endpoint, the live WebSocket, and model availability checks |
| `aistudio.google.com` | API key management page (informational) |
...
- `wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent` - [streaming transcription](/streaming-settings) with Gemini 3.5 Transcribe as the streaming provider
- `https://generativelanguage.googleapis.com/v1beta/models` - model availability checks
...
If you configured **Grok** for post-processing, use the SpaceXAI domains from the previous section.
...
# Test the update installer host
curl -I https://builds.hyperwhisper.com
...
3. **Audio Data**:
   - **HyperWhisper Cloud**: HyperWhisper sends the audio to `transcribe-prod-v2.hyperwhisper.com` on the Fly.io edge network. The service processes the audio in memory and writes no audio to disk. For more information, see [Data Privacy](/data-privacy)
   - **Third-party providers**: HyperWhisper sends the audio directly to the API of the provider, for example SpaceXAI Grok, OpenAI, Groq, or Deepgram. The privacy policy of the provider applies
