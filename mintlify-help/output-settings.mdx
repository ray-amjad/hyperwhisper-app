...
<Note>
  HyperWhisper removes filler words from English transcripts only. This is true on macOS and on Windows, for both regular (mode-based) transcription and [streaming](/streaming-settings#how-streaming-differs-from-modes). A mode whose language is `auto` does not remove filler words, because "er" and "um" are words in other languages. Set the language of the mode to English to use this step.
</Note>
