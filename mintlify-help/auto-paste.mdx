...
3. If an app processes a paste slowly, increase the clipboard restore delay. This delay is a single setting that applies to every app, not a per-app value.
...
- **macOS only:** HyperWhisper does not paste automatically when the target app quits during the recording. It also does not paste automatically when a new app takes the process ID of the target app before the paste. HyperWhisper also does not paste automatically when it did not capture a target app and a different app is in front. This guard keeps the transcript out of the wrong window. The transcript stays on the clipboard, and the app shows no message. The clipboard restore does not run in this case, so the transcript does not disappear after the restore delay. Paste the transcript manually with **⌘V**.
