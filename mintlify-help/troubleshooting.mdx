...
<AccordionGroup>
  ...
  <Accordion title="Bare-modifier push-to-talk not working (macOS)">
    ...
    <Steps>
      ...
      <Step title="Go back to HyperWhisper">
...
<AccordionGroup>
  <Accordion title="No speech detected in the recording">
    The transcription provider analyzed the audio and found no speech. Usually this result is correct, and not a hidden failure.

    The app does a check on the result. After each **No speech detected**, the app measures the audio. If the measurement shows speech, the engine and the recording disagree, and the app sends one report to the crash reporter. This report holds measurements only, and no audio and no text. It goes only if you turn on **Error logging** (macOS) or **Send error reports** (Windows). Read [Data Privacy](/data-privacy#the-no-speech-diagnostic) for the full list.
  ...
  <Accordion title="The speech model was unloaded to free memory (macOS)">
    ...
    For a Whisper model, HyperWhisper first tries to load the model again, and shows the message only if that load also fails. For a Parakeet model, it shows the message immediately.

    HyperWhisper does not retry the transcription automatically. The memory pressure is continuous, so more attempts would only load and lose the model again.

    **Fix:** Close applications that use much memory. Then make the recording again. If the message occurs frequently, select a smaller local model, or select a cloud provider for the mode.

    <Note>
      A [Local API](/local-api) client gets the same condition as the `ENGINE_UNAVAILABLE` error code.
    </Note>
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="HyperWhisper Cloud requires an account key">
    Your mode uses HyperWhisper Cloud, but the app has no account key. HyperWhisper Cloud needs an account key for every request. There is no trial and no anonymous use.

    HyperWhisper stops this transcription on your machine, before the upload. Your audio does not go to the server. The error shows immediately, and it names HyperWhisper Cloud.

    **Fix:** Open **Settings → HyperWhisper Cloud**. Then enter your account key, or click **Get Credits** to buy credits and create a key. Read [Pricing & Cloud Credits](/licensing) for the steps. To transcribe without a Cloud account, change the mode to a local model, or to a provider for which you hold your own API key.
  </Accordion>

  <Accordion title="HyperWhisper Cloud rejects a request with 401 or 403">
    These status codes have different meanings for HyperWhisper Cloud:

    - **401** — the account key is invalid or expired. Open **Settings → HyperWhisper Cloud** and update the key.
    - **403** — HyperWhisper Cloud temporarily blocked the network. Wait a few minutes, then try again. Changing the account key does not correct this network block.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Vocabulary not boosting in streaming">
    Only **HyperWhisper Cloud**, **Deepgram** and **SpaceXAI** support vocabulary boosting during streaming. HyperWhisper Cloud and Deepgram support it only with an explicit language, not Auto. SpaceXAI accepts the terms with any language. ElevenLabs and OpenAI do not support vocabulary boosting in the streaming API.

    If you have vocabulary entries and change to a provider without this support, the streaming settings show a warning.

    **Fix:** Change to HyperWhisper Cloud, Deepgram or SpaceXAI. For HyperWhisper Cloud and Deepgram, also set an explicit language, not Auto. See [Streaming Transcription](/streaming-settings).
  </Accordion>
...
If these fixes do not correct your problem, send an email to [hi@support.hyperwhisper.com](mailto:hi@support.hyperwhisper.com). For a faster diagnosis, include this information:
