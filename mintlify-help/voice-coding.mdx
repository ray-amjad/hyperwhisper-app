...
<Note>
  The Linux app writes a discovery file too, but the bridge does not read the Linux path yet. Linux users can call the local API directly. See [MCP Setup](/api-reference/local-api/mcp-setup#prerequisites).
</Note>
...
| `post_process` | Rewrites text with a preset (`hyper`, `note`, `email`, `commit`, `prompt`), or with your own instruction in the separate `prompt` field. Only `hyper` and `note` map to a real backend preset. The app does not recognize `email`, `commit`, or the `prompt` preset value, and it gives Hyper-style output for all three. |
