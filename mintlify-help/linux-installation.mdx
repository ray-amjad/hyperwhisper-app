...
<Info>
  The current Linux release is v1.14.0. Each Linux release passes the automated
  build, package, UI-smoke, and integration gates. The physical GNOME, KDE, and
  GPU matrix is a recommended checklist, and the release does not stop when the
  matrix is absent.
</Info>
...
Only one copy of HyperWhisper runs at a time. If you launch it again while it
runs, the running copy shows its main window and brings it to the front. The
second launch then exits with no error.
...
A Linux release can include `hyperwhisper-apt-VERSION.tar.gz`, a static APT
...
The release includes this archive only when the build signed the repository. A
release that has no signed repository does not list the archive, and the
download page does not link it. The `.deb` package always ships. If a release
has no `hyperwhisper-apt-VERSION.tar.gz` file, use the `.deb` package, or take
the archive from an earlier release.
