...
A row holds **no account, no key, no request identifier, no IP address, no audio, and no text**. Nothing links two rows to the same person or to the same transcription. HyperWhisper deletes these rows after 1 year. The public page shows only the last 30 days.
