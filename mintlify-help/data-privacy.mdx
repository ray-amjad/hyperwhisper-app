...
- The length of the audio, in whole seconds. For a call that failed, this is an estimate from the size of the audio, because a failed call returns no length.
- Which clip-length group the audio is in: under 10 seconds, 10 to 30 seconds, or more than 30 seconds.
...
A row holds **no account, no key, no request identifier, no IP address, no audio, and no text**. Nothing links a row to a person. The time is rounded to the hour on purpose: without it, the rows of one transcription would share an exact timestamp, and that timestamp would work as a request identifier. HyperWhisper deletes these rows after 1 year. The public page shows only the last 30 days.
