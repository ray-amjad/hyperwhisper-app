...
Error reports go to the crash reporter (Sentry). They are **on** by default on all three platforms. They hold no audio and no text. To stop them, turn off **Error logging** (macOS), **Send error reports** (Windows), or **Send privacy-filtered crash and performance diagnostics** (Linux) in **Settings → General**. Read [General Settings](/general-settings) for that setting.

The switch takes effect immediately, on all three platforms. The app does not need a restart.
