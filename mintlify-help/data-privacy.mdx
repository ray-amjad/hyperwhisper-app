...
Error reports go to the crash reporter (Sentry). They are off unless you turn on **Error logging** (macOS) or **Send error reports** (Windows) in **Settings → General**. Read [General Settings](/general-settings) for that setting.
...
- **Error reports**: these hold no audio and no text. To stop them, turn off **Error logging** (macOS) or **Send error reports** (Windows) in **Settings → General**.
