---
title: App Updates
description: How HyperWhisper updates on macOS, Windows, and Linux, and how to check for updates manually.
---

HyperWhisper checks for updates automatically on macOS and Windows. Linux uses
your distribution's package tools so the app never changes system packages by
itself.

<Tabs>
  <Tab title="macOS">
    HyperWhisper uses [Sparkle](https://sparkle-project.org), the standard auto-update framework for Mac apps.

    ## Automatic updates

    The app checks the appcast at `https://www.hyperwhisper.com/appcast.xml` at launch, and again at intervals while it runs. When a new version is available, the app shows a prompt with the release notes. Click **Install Update**. HyperWhisper then starts again in the new version.

    ## Check for updates manually

    Open the **HyperWhisper** application menu at the top of the screen (the app's own menu, not the menu bar status icon), below **About HyperWhisper**. Then choose **Check for Updates...**. If you have the latest version, the app shows a confirmation. If a new version is available, the update prompt appears.

    ## Turn off automatic checks

    To control the update times yourself, go to **Settings → General**. Then turn off **Auto update**. This control is not a native Sparkle preference. It is a HyperWhisper setting, and the app applies it to the automatic-check behavior of Sparkle. You can still start a manual check at any time with **Check for Updates...**.
  </Tab>
  <Tab title="Windows">
    HyperWhisper uses [NetSparkle](https://github.com/NetSparkleUpdater/NetSparkle), the Windows equivalent of Sparkle.

    ## Automatic updates

    At launch, the app checks the appcast at `https://www.hyperwhisper.com/appcast-windows.xml` for a new version. This feed is separate from the macOS appcast. When a new version is available, the app shows a notification with the release notes and an **Install** button. The new version downloads. Then the app closes and the installer runs automatically.

    ## Check for updates manually

    Right-click the tray icon. Then choose **Check for Updates**. If you have the latest version, the app shows a confirmation. If a new version is available, the update prompt appears.

    ## Turn off automatic checks

    To stop the automatic check in the background, go to **Settings → General**. Then turn off **Auto update**. You can still start a manual check at any time with **Check for Updates**.
  </Tab>
  <Tab title="Linux">
    Open **About**, then click **Refresh package status**. HyperWhisper compares
    the installed package with your distribution's cached package metadata. It
    does not refresh repositories or install packages itself.

    Refresh metadata and install an available version with your distribution's
    normal software updater. For a directly downloaded Debian package, download
    the newer `.deb` and run `sudo apt install ./hyperwhisper_VERSION_amd64.deb`.
  </Tab>
</Tabs>

## Recent updates on the home page (macOS)

The macOS home page shows a **Recent Updates** section at the bottom, below the statistics bar. It reads the same appcast feed and shows the release notes of the 5 most recent versions. The newest version has a **NEW** badge. While the app reads the feed, a loading placeholder appears. If the feed does not answer, the section shows an error and a retry control.

The Windows app has no equivalent section. On Windows, you read the release notes in the update notification window.

## Links in release notes

A release note can contain a link. HyperWhisper makes the link clickable: on macOS in the **Recent Updates** section, on Windows in the update notification window. Click the link to open the page in your browser.

HyperWhisper opens `http`, `https`, and `mailto` links only. The appcast is remote content, thus a link with a different scheme keeps its text but does not become clickable.

## Release channels

HyperWhisper ships one stable channel. There is no separate beta channel or insider channel. We do internal tests on each release before we send it to all users.

If you want a message when a new version ships, follow us on [LinkedIn](https://www.linkedin.com/company/hyperwhisper/).

## Go back to a previous version

If a new release does not work correctly for you, send an email with the details to [hi@support.hyperwhisper.com](mailto:hi@support.hyperwhisper.com). This is the fastest way to get a fix. We can also send you the previous installer while we correct the error.
