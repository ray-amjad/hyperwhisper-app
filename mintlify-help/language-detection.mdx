...
| **Azure MAI-Transcribe** (via HyperWhisper Cloud) | Different for each model: **MAI-Transcribe 2** lists 60 languages, **MAI-Transcribe 1.5** lists 43. The picker shows one less for each model, because HyperWhisper has no picker entry for Odia |
| **Meta Muse Voice Transcribe** (Cloud or direct Meta key) | 25 extensively evaluated languages. Meta's Model API reference does not publish a closed code list |
