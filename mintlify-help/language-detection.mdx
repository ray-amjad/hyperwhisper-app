...
| **Google Gemini 3.5 Transcribe** | Google publishes "85+ languages" and gives no list for each code. Thus the picker shows every language |
| **Azure MAI-Transcribe** (via HyperWhisper Cloud) | Different for each model: **MAI-Transcribe 2** lists 60 languages, **MAI-Transcribe 1.5** lists 42. The picker shows one less for each model, because HyperWhisper has no picker entry for Odia |
| **Meta Muse Voice Transcribe** (Cloud or direct Meta key) | Meta reports 25+ evaluated languages. Its Model API reference does not publish an exact count or a closed code list |
| **Parakeet V3** (macOS) | 25 European languages (English, German, French, Spanish, Italian, Portuguese, Dutch, Polish, Russian, Ukrainian, Czech, Slovak, Hungarian, Romanian, Bulgarian, Croatian, Slovenian, Serbian, Danish, Swedish, Norwegian, Finnish, Estonian, Latvian, Lithuanian) |
| **Parakeet V3** (Windows) | 26 European languages — the macOS list without Russian and Serbian, plus Greek, Catalan, and Basque |
...
**AssemblyAI Dictation** is the second exception, and it works in the opposite direction. Dictation supports 32 languages and it does not support **Automatic**. When you select Dictation, the app does not put a language in its place: it clears the language and you must select one of the 32 languages yourself. Until you do, macOS keeps the **Save** button disabled, and Windows and Linux refuse the save with the message `Dictation needs an explicit supported language and no Medical domain.` HyperWhisper refuses a Dictation request that carries an unsupported language. It does not silently transcribe the audio as English.

The 32 languages are Afrikaans, Arabic, Catalan, Cantonese, Chinese, Danish, Dutch, English, Estonian, Finnish, French, Galician, German, Hebrew, Hindi, Italian, Japanese, Korean, Marathi, Norwegian, Norwegian Nynorsk, Persian, Portuguese, Romanian, Russian, Spanish, Swedish, Turkish, Urdu, Vietnamese, Xhosa, and Zulu.
...
- **Azure MAI-Transcribe** lists Norwegian as `nb`. HyperWhisper Cloud sends `nb` for Norwegian. **MAI-Transcribe 2** also lists Filipino as `fil` and Cantonese as `yue`, and HyperWhisper Cloud sends those codes for it.
- **ElevenLabs Scribe** lists Tagalog as `fil`, and Mandarin as `cmn`.
- **Meta Muse Voice Transcribe** takes a language name for biasing. Cloud and direct Meta requests map a known picker code to that name. They send no language bias for **Automatic** or for a code that they cannot map.
...
Muse can recognize code switching inside one recording. A selected language is still a bias, not a restriction. HyperWhisper does not claim support for a language only because Muse was trained on it. The catalog records 25 evaluated languages until Meta publishes a stable code list.
