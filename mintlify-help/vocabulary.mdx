...
- **Meta Muse Voice Transcribe** — HyperWhisper Cloud sends the terms in Meta's `keywords` field. HyperWhisper keeps the first 100 terms and cuts each term to 80 characters before the request. These are HyperWhisper limits. Meta's API reference does not publish a phrase-count limit. Muse also supports context bias upstream, but HyperWhisper does not expose a separate context-bias control.
