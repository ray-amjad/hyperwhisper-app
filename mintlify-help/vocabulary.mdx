...
- **HyperWhisper Cloud** — the app sends the terms as prompt vocabulary terms. The server keeps the first 100 terms and drops the other terms without a message. The same 100-entry warning banner applies here. If the selected accuracy tier routes to Azure MAI-Transcribe or Google Chirp 3, the server does not send vocabulary hints at all — neither provider supports them through this route.
