...
- **Deepgram** — sent as keyword boosting parameters (up to 100 terms, and terms longer than 50 characters are dropped; a warning appears if you exceed the term limit).
- **OpenAI Whisper, Soniox** — sent as prompt vocabulary terms.
- **HyperWhisper Cloud** — sent as prompt vocabulary terms; silently capped at 100 terms with no warning shown if you exceed it.
...
- **ElevenLabs** — sent as repeated `keyterms` multipart form fields (Scribe v2 only; capped at 100 terms, terms longer than 50 characters or 5 words are dropped; Scribe v1 does not support vocabulary).
- **AssemblyAI** — sent as key-term context (up to 200 terms; phrases longer than 6 words are dropped).
- **Mistral** — sent as a structured `context_bias` field (up to 100 terms, each 80 characters or fewer).
...
Entries without a replacement are injected into the per-request user-message context sent to AI post-processing (entries with a replacement are already applied via regex before post-processing runs, so they aren't re-injected). This helps the model understand your terminology when cleaning up text for Meeting, Custom, and other presets.
