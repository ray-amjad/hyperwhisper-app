...
- **HyperWhisper Cloud** — the app sends the terms as prompt vocabulary terms. The server keeps the first 100 terms and drops the other terms without a message. The same 100-entry warning banner applies here. If the selected accuracy tier routes to Google Gemini 3.5 Transcribe, the server sends the terms in the `custom_vocabulary` field, up to 100 terms, each cut to 80 characters. Azure MAI-Transcribe does support hints on this route: the server sends up to 100 terms, each cut to 50 characters, as a phrase list.
- **Meta Muse Voice Transcribe** — Cloud and direct Meta requests send the terms in Meta's `keywords` field. HyperWhisper keeps the first 100 non-empty terms and cuts each term to 80 characters before the request. These are HyperWhisper limits. Meta's API reference does not publish a phrase-count limit. Muse also supports context bias upstream, but HyperWhisper does not expose a separate context-bias control.
...
- **Gemini 3.5 Transcribe** — the app sends the terms in the `custom_vocabulary` field of the request. This is a real field of the API, and not text in a prompt. The limit is 100 terms, and the app cuts each term to 80 characters. It removes a duplicate term, and it does not compare the capital letters. The same limits apply with your own Google key and through HyperWhisper Cloud. This engine accepts the terms also when the language of the mode is **Auto**.
...
- **AssemblyAI Dictation** — the Dictation model has its own limits. The app sends the terms in the `keyterms_prompt` field. It keeps the first 100 terms, it cuts each term to 80 characters, and it stops when the terms together reach 8000 characters. The same limits apply with your own AssemblyAI key and through HyperWhisper Cloud. Custom vocabulary adds no credits on this model.
- **Grok STT** — the app sends the terms as repeated `keyterm` fields. The limit is 100 terms, and the app drops terms longer than 50 characters. The same limits apply with your own SpaceXAI key, through HyperWhisper Cloud, and in streaming mode.
...
<Warning>
  On **macOS**, four on-device engines apply an earlier, different replacement pass to their own output: **Apple Speech Analyzer**, **Nemotron 3.5**, **Parakeet**, and **Qwen3 ASR**. This earlier pass holds to no word boundaries, and it also ignores accent marks. Therefore an entry such as "eta" also changes the text inside a longer word such as "metadata". Local Whisper on macOS, and all engines on Windows, use only the word-boundary pass above.

  To prevent an unwanted match with these four engines, make the original phrase longer. For example, use "ETA arrival" and not "eta".
</Warning>
