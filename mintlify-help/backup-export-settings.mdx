...
Vocabulary import is **merge-only**: existing words are never wiped, only added to or updated (see [Vocabulary merge behavior](#vocabulary-merge-behavior) below). Mode import works differently: if an incoming mode has the same name as one you already have, the existing mode is deleted and replaced by the version from the file; modes with new names are simply added alongside your existing ones. Only sections present in the chosen file can be restored, and you can deselect any of them before committing.
