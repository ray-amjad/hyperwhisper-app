...
<Tabs>
  ...
  <Tab title="Windows">
    <Steps>
      ...
      <Step title="Record a new key combination">
        Click the shortcut field for the action you want to change. Then press the new key combination. HyperWhisper records the combination when you release the last key. It does not record a key at a time, so a combination that you start and abandon changes nothing.
      </Step>
      <Step title="Correct conflicts">
    ...
    Windows refuses a new combination in four cases. The field keeps the previous value, and the reason appears next to it.

    | Case | Message |
    | --- | --- |
    | A modifier-only shortcut with one modifier | Single modifier shortcuts such as Ctrl, Alt, Shift, or Win are not supported. Use a key with modifiers or a multi-modifier shortcut such as Ctrl+Win. |
    | The combination is already assigned to a different HyperWhisper action | This shortcut is already used for the other action |
    | Another application holds the combination | The shortcut is already in use by another application |
    | Windows reserves the combination | The shortcut is reserved by Windows and cannot be used |

    <Note>
      A modifier-only shortcut needs two modifiers, for example `Ctrl+Win`. One bare modifier would trigger on a normal keystroke such as `Ctrl+C`. Linux applies the same rule.
    </Note>
  </Tab>
  <Tab title="Linux">
...
After the lock starts, HyperWhisper ignores the unlock sequence for a short time. This delay stops a key bounce, or a dropped packet from a wireless keyboard, from unlocking the recording immediately after it locked.

| Platform | Minimum lock time |
| --- | --- |
| macOS | 1 second |
| Windows | 2 seconds |
| Linux | 2 seconds |

If you tap two times before this time, HyperWhisper stays locked. Wait, then tap two times again.
