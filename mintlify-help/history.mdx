...
<Tabs>
  ...
  <Tab title="Windows">
    Go to **Settings → Storage → Automatic Cleanup**. The age limit is a whole number of days (1–365) — there is no minutes/hours picker. A line below the box shows the time of the last cleanup and the number of transcripts that it deleted. The **Delete Now** button runs the cleanup immediately, after a confirmation dialog.
  </Tab>
</Tabs>

For the other settings in this panel, read [Storage Settings](/storage-settings).
