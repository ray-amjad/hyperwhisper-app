---
title: Introduction
description: The HyperWhisper help center. Learn how to use HyperWhisper for AI-powered speech-to-text transcription on macOS, Windows, and Linux.
---

## What is HyperWhisper?

HyperWhisper is a native application for macOS, Windows, and Linux. It transcribes your voice into text with a range of speech-to-text models. HyperWhisper works offline with local models. It also works online with cloud providers such as SpaceXAI Grok, OpenAI, Groq, Deepgram, Assembly AI, ElevenLabs, Mistral, Soniox, and Google Gemini.

## Key Features

<CardGroup cols={2}>
  <Card title="Local or Cloud" icon="server">
    Use local models or cloud APIs
  </Card>

  <Card title="Multiple Languages" icon="globe">
    HyperWhisper supports 99+ languages
  </Card>

  <Card title="HyperWhisper Cloud" icon="cloud">
    A built-in service with 11 speech-to-text engines. There is no markup, and you do not need API keys.
  </Card>

  <Card title="Multiple API Providers" icon="plug">
    Use models hosted on SpaceXAI Grok, OpenAI, Groq, Deepgram, Assembly AI, ElevenLabs, Mistral, Soniox, and Google Gemini. Bring your own API keys.
  </Card>

  <Card title="Custom Modes" icon="list-check">
    Different modes for emails, notes, meetings, and more
  </Card>

  <Card title="Keyboard Shortcuts" icon="keyboard">
    Global keyboard shortcuts for fast access
  </Card>

  <Card title="Custom Vocabulary" icon="book-open">
    Add special terms and names
  </Card>

  <Card title="Auto Paste" icon="paste">
    HyperWhisper pastes the transcribed text into your active application
  </Card>
</CardGroup>

## Getting Started

<CardGroup cols={2}>
  <Card
    title="Transcription Modes"
    icon="sliders"
    href="/transcription-modes"
  >
    Read about the transcription modes and how to customize them
  </Card>

  <Card
    title="API Keys"
    icon="key"
    href="/api-keys"
  >
    Configure API keys for cloud transcription providers
  </Card>

  <Card
    title="Keyboard Shortcuts"
    icon="keyboard"
    href="/keyboard-shortcuts"
  >
    Configure global keyboard shortcuts for fast transcription
  </Card>

  <Card
    title="Auto Paste"
    icon="paste"
    href="/auto-paste"
  >
    Configure automatic paste into your applications
  </Card>
</CardGroup>

## Need Help?

For support, send an email to [hi@support.hyperwhisper.com](mailto:hi@support.hyperwhisper.com). You can also read the documentation sections in the sidebar.
