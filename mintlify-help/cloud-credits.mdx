...
HyperWhisper Cloud is the built-in transcription and post-processing service.
It needs no provider API key. It takes payment from a credit balance, so you
...
Credits are the currency for HyperWhisper Cloud transcription and
post-processing. One credit equals **$0.001 USD**. HyperWhisper deducts the
measured provider cost after a provider processes a request. A request that
stops before a provider call costs no credits.

For transcription, the number of credits depends on the provider that you
select in your mode settings. A new installation starts on ElevenLabs Scribe
v2, which costs about **9.83 credits per minute** of audio. **Settings →
HyperWhisper Cloud** always shows the exact transcription rate next to your
balance. Post-processing uses the input and output token counts of its selected
model.
...
**A different model.** Each provider offers more than one model, and each model has its own rate. For example, OpenAI GPT-4o Mini Transcribe costs approximately half of OpenAI GPT-4o Transcribe. The Microsoft row gives a larger difference: **MAI-Transcribe 2** costs approximately **1.67 credits per minute**, and the older **MAI-Transcribe 1.5** costs approximately **6.0**. The AssemblyAI row gives the largest difference: **Universal-2** costs approximately **2.5**, **Universal-3.5 Pro** approximately **3.5**, and **Dictation** approximately **10.33**. Select the model in the Model row of the mode editor.

<Note>
  **AssemblyAI Dictation** is a flat rate of approximately **10.33 credits per minute**, or **$0.62 per audio hour**. The built-in cleanup is in that rate. Neither custom vocabulary nor the Medical control changes it: Dictation refuses Medical Mode, and it adds no credits for vocabulary. The two SpaceXAI models also share one rate — **Grok Voice Transcribe 2** and **Grok Voice Transcribe 1** both cost approximately **1.67 credits per minute**.
</Note>
