...
**HyperWhisper Cloud** is the default provider. It needs no configuration other than your license. For each other cloud provider, you must add an API key in **Model Library → API Keys**. The provider does not operate without this key. If a key is missing or not valid, a warning appears below the provider picker.
