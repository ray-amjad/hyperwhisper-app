...
Deepgram, Groq, and Grok can report that they processed audio but still return an empty transcript. When this happens on the provider that you selected, HyperWhisper Cloud can try one reachable backup provider. It makes at most one additional provider call for this recovery.

If the backup provider returns text, that provider and its model appear in the result, and its rate applies. If no backup provider returns text, the request stays a **No speech detected** result and costs 0 credits. A provider that reports no audio duration does not start this recovery.
...
The macOS and Windows apps give all provider statuses through the Local API at `GET /health`. This endpoint returns the cached health of each cloud provider and post-processing provider. It also returns information about the installed local models. The endpoint does not start a new probe. It reads the current cached state. A saved Meta key reports `configured` with `reachable: false` because Meta has no safe empty request for key validation. A transcription validates the key for that request, but the health endpoint stays conservative.
