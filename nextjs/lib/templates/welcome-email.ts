...
/**
 * Render the rich HTML version of the welcome email. The layout comes from the
 * shared email building blocks, so it stays consistent with the license and
 * credit emails, while the Loom thumbnail functions as a large play button that
 * deep links to the walkthrough recording.
 */
export const welcomeEmailHtml = (data: WelcomeEmailData) => {
  ...
  return emailDocument({
    ...
    content: `${card(`        <h1 style="color: #2563eb; margin-top: 0; margin-bottom: 16px; font-size: 24px;">Great to have you, ${customerName}!</h1>
...
${infoPanel(`            <p style="margin: 0 0 4px 0; color: #1e40af; font-weight: 600;">Get set up in minutes</p>
        ...
        ${downloadButton(downloadUrl, productName)}`)}
    ...
    footerNote: `This email was sent to ${data.customerEmail} because you requested a ${productName} download link. There is no need to unsubscribe. If you didn't request this, simply ignore it.`,
  });
};
