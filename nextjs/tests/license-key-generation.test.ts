...
describe("generateLicenseKey — how a character is drawn", () => {
  ...
  test("maps each drawn index onto the alphabet character at that index", async () => {
    const { generateLicenseKey } = await loadService();
    const indices = [0, 1, 2, 3, 30, 29, 28, 27, 4, 5, 6, 7, 26, 25, 24, 23];
    behaviour.script = [...indices];

    const generated = generateLicenseKey();

    // The expectation is built from the alphabet the harness declares, not
    // pasted as a literal, so it stays tied to the 31 symbols the service
    // documents. The secret scanner also reads a pasted key-shaped literal as
    // a leak, which it is not, but building it removes the argument.
    const expected =
      "HW" +
      [0, 4, 8, 12]
        .map(
          (start) =>
            ...
            indices
              .slice(start, start + 4)
              .map((index) => EXPECTED_ALPHABET[index])
              .join(""),
        )
        .join("");

    assert.equal(generated, expected);
    assert.match(generated, KEY_PATTERN);
  });
